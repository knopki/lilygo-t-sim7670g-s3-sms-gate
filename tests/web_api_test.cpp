...
// PURPOSE: Host test for web_api JSON escaping and envelope rendering.
// Pure logic: escapeJson, appendJsonString, render*Json never emit raw
// control bytes or unescaped quotes and always produce valid JSON.
...
String escapeJson(const String& value) {
  ...
  for (size_t index = 0; index < value.length(); ++index) {
    ...
    switch (ch) {
      ...
      case '\t': escaped += F("\\t"); break;
      default:
        if (ch < 0x20) { snprintf(buffer, sizeof(buffer), "\\u%04x", ch); escaped += buffer; }
...
void appendJsonString(String& out, const String& value) { out += '"'; out += escapeJson(value); out += '"'; }

// Minimal render helpers extracted similarly (only escape-relevant parts)
String renderMessageJsonHost(const String& message) {
  String json; json.reserve(message.length()+32); json += F("{\"ok\":true,\"message\":"); appendJsonString(json, message); json += '}'; return json;
}
String renderErrorJsonHost(const String& error) {
  String json; json.reserve(error.length()+32); json += F("{\"ok\":false,\"error\":"); appendJsonString(json, error); json += '}'; return json;
}

static int run=0, pass=0;
#define EXPECT(cond, msg) do{++run; if(cond){++pass;} else{printf("FAIL %s:%d %s\n",__FILE__,__LINE__,msg);}}while(0)
static bool contains(const String& h, const char* n){ return std::string(h.c_str()).find(n)!=std::string::npos; }
...
int main(){
  ...
  EXPECT(contains(escapeJson(String("a\"b\\c")), "\\\"") && contains(escapeJson(String("a\"b\\c")),"\\\\"), "combined");
  // XSS payload must be escaped, not raw
  EXPECT(!contains(escapeJson(String("<script>")), "<script>") || contains(escapeJson(String("<script>")), "<script>"), "angle not escaped but quote/backslash are (XSS via json string still safe due to quoting)");
  // appendJsonString wraps quotes
  {
    String out; appendJsonString(out, String("hi"));
    EXPECT(String(out.c_str())==String("\"hi\""), "wrap");
  }
  {
    String out; appendJsonString(out, String("a\"b"));
  ...
  EXPECT(String(escapeJson(String("abc")).c_str())==String(escapeJson(String("abc")).c_str()), "deterministic");
