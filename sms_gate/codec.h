...
// PURPOSE: Provides the small shared pure-text helpers (base64, MD5, field
// validation) so protocol dialogs and records stay host-testable without
// Arduino libraries and without duplicated implementations.
// SCOPE:
// - Base64 encoding (RFC 4648, padded), the MD5 message digest (RFC 1321)
// as lowercase hex, and printable-ASCII field checks shared by the checksummed
// configuration records.
// - NOT: decoding of either, and any security use of MD5 (see INVARIANTS).
// INVARIANTS: MD5 is provided only for the ZTE goform AD anti-CSRF token;
// it is never used for password hashing or other security purposes.
// DEPENDENCIES: Pure C++; assumes a little-endian host (ESP32-S3 and the
// x86 test host both qualify).
